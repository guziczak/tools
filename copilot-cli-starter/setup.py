#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Setup script for GitHub Copilot CLI Docker image."""
...
class SetupConfig:
    ...
    no_cache: bool = False
    image_name: str = "copilot-cli-container"
    container_name: str = "copilot-persistent"
...
class ImageSetup:
    """Main setup class for Copilot CLI Docker image."""
    ...
    def run(self) -> None:
        ...
        logger.info("=== GitHub Copilot CLI Docker Image Setup ===\n")
    ...
    def _check_docker_backend(self) -> bool:
        ...
        try:
            ...
            if is_hyperv and not is_wsl:
                logger.warning("\nDocker is using Hyper-V instead of WSL 2!")
    ...
    def _ensure_container(self) -> None:
        ...
        if self.docker_checker.container_exists(self.config.container_name):
            ...
            # Container will be created on first run of copilot.py
            logger.info("Container will be created on first Copilot session")
    ...
    def _display_instructions(self) -> None:
        ...
        copilot_py_path = current_dir / "copilot.py"

        logger.info("\nImage built successfully!")
        logger.info("\n=== Persistent Container Architecture ===")
        logger.info("Copilot CLI uses a single persistent container where:")
        ...
        if ' ' in str(copilot_py_path):
            logger.info(f'   python "{copilot_py_path}"\n')
        else:
            logger.info(f"   python {copilot_py_path}\n")
        ...
        logger.info("\nUsage:")
        logger.info(f"  python {copilot_py_path}              # Run Copilot CLI in current directory")
        logger.info(f"  python {copilot_py_path} [command]    # With arguments")

        logger.info("\nProject Isolation:")
        logger.info("  - Each Copilot session sees ONLY the current project directory")
        ...
        logger.info("\nDocker volumes:")
        logger.info("  - copilot-shared-tools: User-installed packages")
        logger.info("  - copilot-apt-cache: APT package cache")
        logger.info("  - copilot-usr-local: System-wide installations")
        logger.info("  - Project directory: Mounted isolated in container")

        logger.info("\nGitHub Copilot CLI Features:")
        logger.info("  - AI-powered coding assistance in terminal")
        logger.info("  - Multiple AI models (Claude Sonnet 4.5, GPT-5, etc.)")
        logger.info("  - Agentic capabilities - edits files, runs commands")
        logger.info("  - MCP (Model Context Protocol) support")
        logger.info("  - GitHub integration (issues, PRs, search)")
        logger.info("  - Preview every action before execution")
        logger.info("")
        logger.info("Authentication:")
        logger.info("  On first run, Copilot will open browser for GitHub OAuth.")
        logger.info("  Just log in - no tokens or API keys needed!")

        logger.info("\nReady to use! Just run copilot.py from any project directory.")
